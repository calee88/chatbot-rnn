...
def train(args):
    ...
    if not os.path.exists(args.save_dir):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            # with open(os.path.join(args.save_dir, 'config.pkl')) as f:  #Original
            with open(os.path.join(args.save_dir, 'config.pkl'), 'rb') as f:  #new
    ...
    #with open(os.path.join(args.save_dir, 'config.pkl'), 'w') as f: # original
    with open(os.path.join(args.save_dir, 'config.pkl'), 'wb') as f:  # New
        cPickle.dump(args, f)
        ...
        try:
            ...
            for e in range(*epoch_range):
                ...
                for b in range(*batch_range):
                    ...
                    print ("{}/{} (epoch {}/{}), loss = {:.3f}, time/batch = {:.3f}s".format(b, batch_range[1], e, epoch_range[1], train_loss, elapsed))
